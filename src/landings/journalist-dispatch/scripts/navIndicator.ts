// Journalist Dispatch — GSAP scroll-aware navigation indicator.
//
...
// Below that the mobile drawer takes over and there is no shared pill to
...
if (container && indicator && links.length > 0) {
  ...
  mm.add(
    ...
    (context: gsap.Context) => {
      ...
      if (!isDesktopNav) {
        // Mobile/tablet: the drawer nav owns navigation and the pill stays
        // hidden via Tailwind's `nav:block`. Only aria-current tracking is
        // needed here — no indicator geometry to compute or animate.
