// Aperture Editions — GSAP scroll-aware navigation indicator.
//
...
// anchors; the indicator is purely decorative (aria-hidden, see
// Header.astro) and the active link's accessible state is kept in sync via
// aria-current.
//
// Scoped to the `nav` breakpoint (960px, matching Tailwind's `nav:` variant
// where the desktop nav row — and the indicator itself — becomes visible).
// Below that the mobile panel takes over and there is no shared underline to
// animate, so only aria-current tracking runs there.
...
if (container && indicator && links.length > 0) {
  ...
  mm.add(
    ...
    (context: gsap.Context) => {
      ...
      if (!isDesktopNav) {
        // Mobile/tablet: the panel nav owns navigation and the underline
        // stays hidden via Tailwind's `nav:flex`. Only aria-current tracking
        // is needed here — no indicator geometry to compute or animate.
